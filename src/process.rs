impl PgBouncer {
    fn new(group: Group, index: usize) -> PgBouncer {
        PgBouncer {
            group_id: group.id,
            index,
            port: 6432,
    ...
    fn pidfile(&self) -> Result<PathBuf> {
    ...
    fn admin(&self, command: &str) -> Result<()> {
        let dir = self.dir()?;
        let dir_str = dir.to_str().context("no utf8")?;
